...
#
# DOES:
#   -updates framework codes on ALL biblios based on hardcoded map of LEADER values, if --update is set
#
# CREATES:
#   -nothing
#
...
while (my $bib=$bib_sth->fetchrow_hashref()) {
   ...
   my $rec = GetMarcBiblio($this_biblio);
