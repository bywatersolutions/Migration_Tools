...
#      <item barcode>,<datatoupdate>
#
# DOES:
#   -updates the associated line in the issues table, if --update is set
#
# CREATES:
#   -nothing
#
...
GetOptions(
   'in:s'     => \$infile_name,
   'field=s'  => \$fieldtoupdate,
   'debug'    => \$debug,
   'update'   => \$doo_eet,
);

if (($infile_name eq q{}) && (!$fieldtoupdate) ){
...
FIELD:
   ...
   if ($doo_eet){
         my $update_sth =$dbh->prepare("UPDATE issues SET $fieldtoupdate = ? WHERE itemnumber = $rec->{'itemnumber'}");
         $update_sth->execute($data[1]);
         $written++;
   }
}
