...
# -------------------------------------------------------------------------- #
# DO NOT USE TRUNCATE - THIS REMOVES THE WHOLE TABLE INCLUDING THOSE         #
# DEFAULT PREFERENCES YOU WILL NEED TO POPULATE THE BORROWERS RECORDS WITH.  #
# -------------------------------------------------------------------------- #
...
sub force_borrower_messaging_defaults {
       ...
       my $sth = $dbh->prepare("SELECT borrowernumber, categorycode FROM borrowers WHERE dateenrolled >= ? and categorycode='AR'");
       $sth->execute($since);
       while ( my ($borrowernumber, $categorycode) = $sth->fetchrow ) {
           print "$borrowernumber: $categorycode\n";
           next unless $doit;
           my $err = C4::Members::Messaging::SetMessagingPreferencesFromDefaults( {
