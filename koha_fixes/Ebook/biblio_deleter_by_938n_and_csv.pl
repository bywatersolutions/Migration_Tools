...
#Read the database, and if an 938$n is present and that substring of 938n is in the csv
...
while (my $row = $sth->fetchrow_hashref()){
  ...
  next RECORD if (!$record->subfield('938','n'));

  foreach my $tag ($record->field(938)){
   my $subn = $tag->subfield('n') || "NONE";
   my $final_tag=$subn;
   ...
   if ((exists $ebooks{$final_tag}) && $doo_eet){
     ...
     if ($doo_eet){
       if ($itemnum) {
         ...
         print "\n DELETED itemnumber: $itemnum for $final_tag\n";
          $itemdel++;
        }
        C4::Biblio::DelBiblio($bibnum);
        print "\n DELETED  $final_tag with bibnumber $bibnum \n";
        $deleted++;
     }
   }
 }
...
print "\n\n$deleted records deleted.($itemdel items deletd)\n";
