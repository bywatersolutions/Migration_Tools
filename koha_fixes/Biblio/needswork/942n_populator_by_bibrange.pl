...
my $sth=$dbh->prepare("SELECT DISTINCT biblionumber,frameworkcode from biblio 
                       WHERE biblionumber between ? and ?");
$sth->execute( $bibstart,$bibend );

RECORD:
while (my $thisrec=$sth->fetchrow_hashref()){
   last RECORD if ($debug and $modified > 4000);
   $i++;
   print ".";
...
$debug and print "record $thisrec->{'biblionumber'} found with field 942 value $curval\n";
      if ($curval ne $val){
...
$debug and print "no field found for  $thisrec->{'biblionumber'} \n";
