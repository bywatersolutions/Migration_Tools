...
while (my $line=$csv->getline($infl)){
   ...
   if ($data[0] =~ m/[0-9A-Z]+/) {
      ...
      $borr = $data[0];
      $itembar = $data[1];
     if ($data[2] =~ m/[0-9]+\/[0-9]+\/[0-9]+/ ) {
       $datedue = format_the_date($data[2]);
       $issued = format_the_date($data[2])-14;
     }
$debug and print "$borr, $itembar, $datedue and $issued\n";
     if (!$debug) {
     print $outfl $borr.','.$itembar.','.$datedue.','.$issued.','.$branch."\n";
     }
     next LINE;
     $written++;
   }
