...
#                    by MARC::Record->field(); i.e. "9.." will drop all 9xx fields, while "952" will only drop 952.  Multiple
#                    entries are allowed.
#   --branchdefault=<data>  Inserts data into 952$a and 952$b for each item record
...
#           date     Tidies up dates, mm/dd/yyyy  any delimiter, renders in ISO form
#           date2    Tidies up all-numeric dates yyyymmdd, renders in ISO form
#           date8    Tidies up epoch time (time started 1/1/1970), renders in ISO form
#           date7    Tidies up dd mm yyyy (any delimiter), renders in ISO form
#           date6    Tidies up yyyy mm dd (any delimiter), renders in ISO form
#           date2    Tidies up yyyymmdd (NO delimiter), renders in ISO form
#           date3    Tidies up MONTHname dd yyyy (with or without comma after dd), renders in ISO form
#           date5    Tidies up MONTHname-dd-yyyy (with dash separator), renders in ISO form
#           TODO!!:    Analyze date6a and date4: these appear to duplicate original date tools.
...
#open my $output_file,'>',$output_marc_filename;
...
#$debug and print "DANI THE CALLNUMBER IS::::   $default_callnumber\n";
   my $default_price = 0;
...
#my $barcode = $field_in->subfield('g');
#print "$barcode\n";
   ...
   if ($tool eq 'date8') {
      $data =~ s/ //g;
       if ($data && $data ne 'NULL') {
          ...
          my ($year, $month, $day) = Add_Delta_Days(1970, 1, 1, $data);
          if ($month && $day && $year){
...
#date4 is mm-dd-yyyy or mm/dd/yyyy
   if ($tool eq 'date4') {
      $data =~ s/ //g;
