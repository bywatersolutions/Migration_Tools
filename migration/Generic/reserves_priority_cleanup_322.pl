...
# -D Ruth Bavousett
#
# updated on 5/13/2016 to remove constraint type field from query.  That field is 
#   not used in 3.22 onward
#---------------------------------
...
my $reserves_sth = $dbh->prepare("SELECT borrowernumber, reservedate FROM reserves
                                  WHERE  biblionumber   = ?
...
while (my $biblio = $biblio_sth->fetchrow_array ){
   $debug and last BIBLIO if $i > 100;
   $i++;
   print ".";
