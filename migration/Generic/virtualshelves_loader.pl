...
#   my ($mapsub,$map_filename) = split (/:/,$map);
   ...
   open my $mapfile,'<',$borrmap_filename;
   while (my $row = $csv->getline($mapfile)) {
      my @data = @$row;
      $borrmap{$data[0]} = $data[1];
   }
   close $mapfile;
}
...
while (my $line=$csv->getline($io)){
   $debug and last if ($j>5000); 
   $j++;
   print ".";
   print "\r$j" unless ($j % 100);
   my @data = @$line;
   $debug and print Dumper(@data);
   my $querystr = "INSERT INTO virtualshelves (";
   my $exception = 0;
   ...
   for (my $i=0;$i<scalar(@fields);$i++){
      ...
      if ($fields[$i] eq $borrowercol){
         if ($borrmap{$data[$i]}) {
            $debug and say "Mapped: patron number $data[$i] to $borrmap{$data[$i]}\n";
            $data[$i] = $borrmap{$data[$i]};
         }
