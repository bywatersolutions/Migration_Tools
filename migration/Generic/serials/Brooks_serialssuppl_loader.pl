...
my $sub_insert_sth = $dbh->prepare("INSERT INTO subscription (biblionumber, librarian, branchcode, notes,  
                                     location, issuesatonce, serialsadditems, graceperiod, status,
                                     callnumber, internalnotes, staffdisplaycount,opacdisplaycount, periodicity, numberlength) 
                                    VALUES (?,'koha','BROOKSBRAT','The current issue of this magazine does not circulate.',
                                            ?,1,1,14,1,
                                            ?,?,99,99,?,?)");
...
my $add_items_sth = $dbh->prepare("INSERT INTO items (biblionumber, biblioitemnumber, barcode, homebranch, holdingbranch,
                                  notforloan, damaged,itemlost,wthdrawn,location, enumchron, itemcallnumber, itype)
                                  VALUES (?,?,?,'BROOKSBRAT','BROOKSBRAT',0,0,0,0,?,?,?,?)");
...
#$debug and print "$loc and $location\n";
   my $callnum  = $line->{'852subk'} || " ";
   my $total_callnum = $callnum . " " . $loc;
#$debug and print "$total_callnum\n";
   my $subscrip_note =  $line->{'852subx'}  || " ";
   my $volume = ($line->{'864i'}. ':' . $line->{'864j'}) || "Supplement";
   my $volume_note = $line->{'864x'} || " ";
   $internalnote = $itype ." ". $loc ." ". $callnum ." ". $subscrip_note;
   my $barcode = $line->{'864p'} || " ";
   if ($barcode eq " ") {
     $barcode = "AUTO" . $i;
   }
...
#       $subscription_id = $addedsubscription_id;
       $hist_insert_sth->execute($biblionumber, $addedsubscription_id, $subscrip_note, $volume);
       $add_serial_sth ->execute($biblionumber, $addedsubscription_id, $volume, $volume_note, $barcode);
      $serials++;
      $written++;
   }
    ...
    $add_serial_sth ->execute($biblionumber, $subscription_id, $volume, $volume_note, $barcode);
    $serials++;
    }
