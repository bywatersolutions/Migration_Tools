...
#
# -D Ruth Bavousett
#drnoe mod #4 of table_updater.pl in an attempt to map serialid or biblionumbers
#---------------------------------
...
GetOptions(
    ...
    'item=s'   => \$itemcol,
    'bib=s'    => \$bibliocol,
    'serial_map=s'        => \$serial_map_filename,
...
print "Reading in serial map file.\n";
if ($serial_map_filename ne $NULL_STRING) {
   my $csv = Text::CSV_XS->new();
   open my $serial_mapfile,'<',$serial_map_filename;
   while (my $line = $csv->getline($serial_mapfile)) {
      my @data = @$line;
      $serial_map{$data[0]} = $data[1];
   }
   close $serial_mapfile;
}
...
$debug and print "OLD VALUE: $oldval\n";
      if ( exists($serial_map{$oldval}) ) {
         $debug and print "MAPPED: $oldval  TO $serial_map{$oldval}\n";
         $data[0] = $serial_map{$oldval};
      }
