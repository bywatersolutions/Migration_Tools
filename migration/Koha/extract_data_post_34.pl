...
# -D Ruth Bavousett
# -Joy Nelson
#  4-8-2017 verified that it uses GetMarcBiblio instead of select marc from biblioitems
#
#---------------------------------
#
...
# EXPECTS:
#   -a branchcode, or 'ALL'
#   -optional maps for branches, shelflocs, itypes, collcodes, or patron categories 
#
# DOES:
#   -nothing
#
...
my $branch               = $NULL_STRING;
my $branch_map_name      = $NULL_STRING;
my $shelfloc_map_name    = $NULL_STRING;
my $itype_map_name       = $NULL_STRING;
my $collcode_map_name    = $NULL_STRING;
my $patron_map_name      = $NULL_STRING;
my $patron_code_map_name = $NULL_STRING;
my $tag_itypes_str       = $NULL_STRING;
my $frames_to_dump       = $NULL_STRING;
my $skip_biblio          = 0;
my %branch_map;
my %shelfloc_map;
my %itype_map;
...
if ($frames_to_dump ne $NULL_STRING){
...
while (my $prerow = $pre->fetchrow_hashref()){
   ...
   my $marc = GetMarcBiblio($prerow->{biblionumber},1);
   next if (!$marc);
   my $tag_this_biblio=0;
   my $tag_branch=$NULL_STRING;
